...
# Create a dictionary with bigrams as the keys and a list of "following
# words" as the values.
...
def bigram_dictionary(training_text):
    ...
    for word in training_text:
        ...
        if pos <= (length - 3):
            ...
            # in case where bigram is already in dictionary, we want to append
            # the "following word" to the value
            if dict_key in bigram_dict:
                # append following word to list for that dict_key
                bigram_dict[dict_key].append(training_text[pos + 2])
            # when bigram is not yet in dictionary, add it as key along with
            # its value
            else:
...
def create_text(bigram_dict):
    ...
    starts_of_sentences = [
        key for key in bigram_dict.keys() if key[0][0].isupper()]
    ...
    while current_state in bigram_dict and i < 5000:
        ...
        # look up the current state in our bigram dictionary, and choose the
        # following word.
